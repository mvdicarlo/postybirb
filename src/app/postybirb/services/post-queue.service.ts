...
export class PostQueueService {
  ...
  public dequeue(id: number): void {
    ...
    if (index !== -1) {
      ...
      this.queue[index].formData.websites = [...this.queue[index].formData.websites, ...this.queue[index].postStats.fail];
    ...
    if (postingSubmission) {
      ...
      this.timeout = setTimeout(function() {
        ...
        .then((data) => {
          ...
          if (err.msg) {
            this.snotify.error(err.msg, `${postingSubmission.title || 'Untitled'}-${website}`, {
              timeout: 10000
            });
          }
        }).finally(() => {
          ...
          if (!postingSubmission.formData.websites.length) {
            ...
            if (postingSubmission.postStats.fail.length) {
              ...
              this.postingSubmission.cleanUp();
    ...
    return new Promise((resolve) => {
      ...
      .then(thumbnail => {
        this._translate.get(failed ? 'Failed' : 'Success', ).subscribe((msg) => {
          new Notification(submission.title, {
            body: `${submission.title} - ${msg}`,
            icon: thumbnail.buffer
          });

          failed ? this.snotify.error(`${msg} (${submission.postStats.fail.join(', ')})`, { timeout: 30000, showProgressBar: true })
          : this.snotify.success(msg, { timeout: 10000, showProgressBar: true });
        })
      }).finally(() => resolve())
    });
  }
