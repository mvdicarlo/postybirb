import { Component, OnDestroy, Output, EventEmitter, ChangeDetectorRef, ChangeDetectionStrategy, ViewChild } from '@angular/core';
...
@Component({
  ...
  host: {
    '[class.d-none]': 'submissions.length < 2'
  }
})
export class CopySubmissionSelectComponent implements OnDestroy {
  ...
  constructor(private _store: Store, private _changeDetector: ChangeDetectorRef) {
    this.stateSubscription = _store.select(state => state.postybirb).pipe(debounceTime(100)).subscribe(submissions => {
      const allSubmissions: SubmissionArchive[] = [...submissions.editing, ...submissions.submissions]
      this.submissions = allSubmissions.map((archive: SubmissionArchive) => {
        return {
