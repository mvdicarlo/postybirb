...
function validate(submission: Submission, formData: SubmissionFormData): any[] {
  const problems: any[] = [];
  const tags = getTags(submission, Furbooru.name);
  if (tags.length < 5) problems.push(['Requires minimum tags', { website: 'Furbooru', value: 5 }]);
  if (!supportsFileType(submission.fileInfo, ACCEPTED_FILES)) {
    problems.push(['Does not support file format', { website: 'Furbooru', value: submission.fileInfo.type }]);
  }
  ...
  if (MBtoBytes(100) < submission.fileInfo.size) {
    problems.push(['Max file size', { website: 'Furbooru', value: '100MB' }]);
  }
...
@Website({
  ...
  acceptsSrcURL: true,
  displayedName: 'Furbooru',
  login: {
    url: 'https://furbooru.org/users/sign_in'
  },
  components: {
    submissionForm: FurbooruSubmissionForm
  },
  validators: {
    submission: validate
  },
...
export class Furbooru extends BaseWebsiteService {
  readonly BASE_URL: string = 'https://furbooru.org';
