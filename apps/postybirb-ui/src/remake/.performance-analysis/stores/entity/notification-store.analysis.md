...
## Potential Issues
- **⚠️ Duplicate websocket subscription** — `NOTIFICATION_UPDATES` is subscribed to in two places: once by `createEntityStore` (updates store) and once by the module-level `AppSocket.on(NOTIFICATION_UPDATES, ...)` that calls `showUINotification`. Both handlers fire on every event. The `showUINotification` handler iterates ALL notifications in the payload, not just new ones.
- **`useUnreadNotificationCount` does a full filter without `useShallow`** — returns a primitive (number), so `useShallow` isn't needed for the return value, but the filter still runs on every update. Since `state.records` changes on every update, this selector always re-evaluates.
...
## Recommendations
- Deduplicate the websocket handler — handle UI notifications inside the store's `setRecords` or a middleware, not via a separate socket listener.
- The `showUINotification` side-effect iterating all notifications on every push could be optimized to only process genuinely new ones (compare against previous set).
