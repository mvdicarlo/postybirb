...
import {
  Insert,
  PostEventRepository as LibPostEventRepository,
  PostEvent,
...
export class PostEventRepository {
  ...
  async insert(event: Insert<'PostEventSchema'>): Promise<PostEvent> {
    const inserted = (await this.repository.insert(event)) as PostEvent;
    const postRecord = await this.postRecordRepository.findById(
      event.postRecordId,
    );
    if (postRecord) {
      this.submissionEventPublisher?.markChanged(postRecord.submissionId);
    }
    return inserted;
  }
}
